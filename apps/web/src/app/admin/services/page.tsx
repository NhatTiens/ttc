...
export default function AdminServicesPage() {
  ...
  return <div className="admin-page"><PageHeader eyebrow="ADMIN / SERVICES" title="Dịch vụ" description="Quản lý catalog và giá bán cho khách hàng. Provider cost chưa thuộc Work 05." actions={<Link href="/admin/services/new" className="button button--primary button--md">Tạo dịch vụ</Link>} />
    <Card className="admin-filter-card"><div className="admin-filter-grid"><SearchInput value={search} onChange={(event) => { setSearch(event.currentTarget.value); setPage(1); }} placeholder="Tên hoặc mã dịch vụ..." /><Select value={platform} onChange={(event) => { setPlatform(event.currentTarget.value); setPage(1); }} options={[{ value: "all", label: "Tất cả nền tảng" }, ...["FACEBOOK","TIKTOK","INSTAGRAM","YOUTUBE","THREADS"].map((value) => ({ value, label: value }))]} /><Select value={category} onChange={(event) => { setCategory(event.currentTarget.value); setPage(1); }} options={[{ value: "all", label: "Tất cả danh mục" }, ...(categories.data ?? []).map((item) => ({ value: item.id, label: item.name }))]} /><Select value={status} onChange={(event) => { setStatus(event.currentTarget.value); setPage(1); }} options={[{ value: "all", label: "Tất cả trạng thái" }, { value: "ACTIVE", label: "Hoạt động" }, { value: "MAINTENANCE", label: "Bảo trì" }, { value: "DISABLED", label: "Vô hiệu" }]} /></div></Card>
