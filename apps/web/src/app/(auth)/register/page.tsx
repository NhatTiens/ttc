...
export default function RegisterPage() {
  ...
  async function submit() {
    ...
    try {
      ...
      toast({ tone: "success", title: "Account created", description: "Registration succeeded in the mock repository." });
      router.push("/dashboard");
    } catch (reason) {
      toast({ tone: "error", title: "Registration failed", description: reason instanceof Error ? reason.message : "Vui lòng thử lại." });
    } finally { setSubmitting(false); }
  }

  return <AuthShell title="Tạo tài khoản" description="Đăng ký để sử dụng dịch vụ và theo dõi đơn hàng." footer={<span>Đã có tài khoản? <Link className="text-link" href="/login">Đăng nhập</Link></span>}><div className="auth-form"><Input label="Full name" autoComplete="name" value={name} error={errors.name} onChange={(event) => { setName(event.currentTarget.value); setErrors((current) => ({ ...current, name: undefined })); }} /><Input label="Email" type="email" autoComplete="email" value={email} error={errors.email} onChange={(event) => { setEmail(event.currentTarget.value); setErrors((current) => ({ ...current, email: undefined })); }} /><Input label="Password" type="password" autoComplete="new-password" value={password} error={errors.password} onChange={(event) => { setPassword(event.currentTarget.value); setErrors((current) => ({ ...current, password: undefined })); }} /><Input label="Confirm password" type="password" autoComplete="new-password" value={confirm} error={errors.confirm} onChange={(event) => { setConfirm(event.currentTarget.value); setErrors((current) => ({ ...current, confirm: undefined })); }} /><label className="checkbox-row checkbox-row--stack"><input type="checkbox" checked={accepted} onChange={(event: { currentTarget: { checked: boolean } }) => { setAccepted(event.currentTarget.checked); setErrors((current) => ({ ...current, accepted: undefined })); }} /><span>Tôi đồng ý với điều khoản dịch vụ và chính sách bảo mật.</span></label>{errors.accepted ? <span className="field-error" role="alert">{errors.accepted}</span> : null}<Button size="lg" className="full-width" onClick={submit} loading={submitting}>Đăng ký</Button></div></AuthShell>;
}
