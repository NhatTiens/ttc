...
export function Sidebar({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <aside className={cn("sidebar", compact && "sidebar--compact")} aria-label="Điều hướng chính">
      <div className="sidebar__brand"><BrandMark compact={compact} /></div>
      <nav className="sidebar__nav">
        <p className="sidebar__section-label">TRÌNH ĐƠN</p>
      ...
      <div className="sidebar__bottom">
        ...
        <Link href="/support" className="sidebar__help" title={compact ? "Hỗ trợ" : undefined}><SupportIcon size={18} />{!compact ? <span><strong>C\u1ea7n h\u1ed7 tr\u1ee3?</strong><small>Ph\u1ea3n h\u1ed3i trong gi\u1edd l\u00e0m vi\u1ec7c</small></span> : null}</Link>
      </div>
    </aside>
  );
}
